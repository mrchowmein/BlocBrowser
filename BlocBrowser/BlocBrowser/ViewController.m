...
- (void)loadView {
    ...
    self.textField.placeholder = NSLocalizedString(@"Google Search or Type URL", @"Placeholder text for web browser URL field");
    ...
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"Welcome", @"Greeting")
                                                                   message:@"BlocBrowser"
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    if ([URLString componentsSeparatedByString:@" "].count > 1){
        NSString *newURLString = [URLString stringByReplacingOccurrencesOfString:@" " withString:@"+"];
...
- (void) floatingToolbar:(AwesomeFloatingToolbar *)toolbar didSelectButtonWithTitle:(NSString *)title {
    if (kWebBrowserBackString) {
        [self.webView goBack];
    } else if (kWebBrowserForwardString) {
        [self.webView goForward];
    } else if (kWebBrowserStopString) {
        [self.webView stopLoading];
    } else if (kWebBrowserRefreshString) {
        [self.webView reload];
    }
}
